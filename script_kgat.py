...
def train(args):
    ...
    user_ids = list(data.test_user_dict.keys()) # for test
    user_ids_batches = [user_ids[i: i + args.test_batch_size] for i in range(0, len(user_ids), args.test_batch_size)] # for test
    user_ids_batches = [torch.LongTensor(d) for d in user_ids_batches] # for test
    if use_cuda:
        user_ids_batches = [d.to(device) for d in user_ids_batches] # for test
